...
- **Cuatro trampas técnicas evitadas a propósito** (documentadas en `.planeacion/2026-07-30_panoptico_plan.html`, pestaña 02): (1) una consulta agregada por métrica — 14 consultas cubren el catálogo completo, constantes sin importar la ventana de 7/30/90 días, nunca un ciclo con un conteo por día; (2) el día se trunca en `America/Mexico_City`, no en UTC — `date_trunc` ingenuo corta la noche (el pico de actividad) al día siguiente, y las gráficas se ven igual de razonables estando mal; (3) ningún desglose (por subforo, por segmento) expone un grupo con menos de 5 elementos — se colapsa en un cubo "Otros"; (4) la carga por moderador es la única pieza visible a `MOD`, y solo como número propio + promedio del equipo — el desglose por persona es `ADMIN`, para no convertir una herramienta de trabajo en un tablero de comparación entre compañeros.
...
- **La única tabla propia del panóptico, y por qué se aceptó (ciclo 13A).** `ConteoAtribucion` guarda cuántos intentos de atribución de alta hubo cada día y en qué escalón se cayeron. Existe porque el evento que mide **no deja rastro en ninguna otra tabla**: un intento incrementa el contador de invitaciones de alguien más y desaparece; su única huella era una línea de log, que en Render se pierde con cada reinicio del contenedor. El total de éxitos sí sobrevivía (`SUM(User.invitaciones)`), pero sin denominador ni causas de descarte, "13 altas y 0 atribuidas" no se puede distinguir de un mecanismo roto. **Es un conteo por día y no una bitácora de eventos a propósito:** una fila por intento con su marca de tiempo permitiría emparejar cada alta con su invitación por cercanía temporal, que es justo la reconstrucción por correlación que el contador ciego del 11A cerró. La tabla no tiene identidades de ningún tipo, y una prueba lo asserta **contra el esquema** para que no se cuele una "solo para depurar". Su desglose es también la única excepción a la trampa 3: los segmentos son escalones de un recorrido, no grupos de personas, y colapsarlos bajo 5 borraría lo único que el indicador contesta sin proteger a nadie.
