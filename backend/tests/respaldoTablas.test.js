...
module.exports = async function run() {
  ...
  // La propiedad que de verdad importa el día que haya que restaurar: el orden
  // de MODELOS es el orden de inserción, y una tabla no puede ir antes que
  // aquellas a las que apunta por llave foránea obligatoria.
  const fueraDeOrden = [];
  enMapa.forEach((tabla, i) => {
    for (const dep of DEPENDE_DE[tabla] || []) {
      const j = enMapa.indexOf(dep);
      if (j === -1 || j > i) fueraDeOrden.push(`${tabla} (posición ${i}) va antes que ${dep} (${j})`);
    }
  });
  check('el orden de MODELOS es restaurable: nadie va antes que aquello de lo que depende',
