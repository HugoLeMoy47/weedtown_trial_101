...
//
// El CTA agrega `?ref=post` y a veces `&next=<ruta>` a la URL de /login. Ese
// estado tiene que sobrevivir tres flujos de autenticación muy distintos:
...
export function capturarAtribucion(searchParams) {
  ...
  if (ref) {
    try { localStorage.setItem(REF_KEY, ref); } catch { /* modo privado sin storage */ }
  }
  if (next) {
